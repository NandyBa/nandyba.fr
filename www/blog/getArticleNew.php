...
<!-- Post -->
	...
	<article class="post">
		<header>
			...
			<div class="meta">
				...
				<a title="lien vers l'auteur de l'article" rel="nofollow" href="https://nandyba.fr/cv.php" class="author"><span class="name">Nandy Bâ</span><img src="../public/images/tinny/nandy@50.jpg" alt="photo de Nandy Bâ" /></a>
				<?php else: ?>
				<a title="lien vers l'auteur de l'article" rel="nofollow" class="author"><span class="name"><?= $article['metaAuthor'] ?></span><img src="../public/images/avatar.jpg" alt="" /></a>	
				<?php endif; ?>
			</div>
		</header>
	...
	if(!empty($article['sortUrl'])){
		$View->SendCanonical('https://nandyba.fr/blog/'.$article['sortUrl']);

		if(!empty($article['imageUrl'])){
			$View->addHeaderImg($article['imageUrl']);
	...
	$Header->SendMaintenanceHeader();
	ob_start();
	?>
	<!-- Ploff  -->
	<article class="post">
		<p>Article en écriture, il devrait paraître dans environ une semaine désolé la gêne occassionée </p>
		<!-- <footer>
			<ul class="stats">
				<li><a title="" href="#">General</a></li>
				<li><a title="" href="#" class="icon fa-heart">28</a></li>
